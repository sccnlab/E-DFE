...
#	print('testing123')
#	print('here is the target and output', ctx.output[0].size(), ctx.target.size()) 
...
def best_meter(ctx: Context, train_ctx: Context, best_branch: int = 1, k: int = 1) -> float:
	...
	def accuracy(output, target, k=1):
		batch_size = target.size(0)
		print('accuracies', target.size(), output.size())
		_, pred = output.topk(k, 1, True, True)
		pred = pred.t()
		correct = pred.eq(target.view(1, -1).expand_as(pred))
...
@register
def custom_acc(output: tuple, target:tuple) -> dict:
	"""custom accuracy
	"""
